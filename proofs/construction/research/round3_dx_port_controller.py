"""Occupation-aware adversarial-port discovery for uniform end controllers.

The two caps are disjoint 5 x k rectangles. External mirrored play may delete
A(v), B(tau(v)) at any inward port v. Nonmirror White replies are allowed
away from inward ports, or at an inward port whose reflected exterior
Blue cell is already forbidden. Thus they create no exterior mirror defect.
Compared with round two, cap stone occupations block the corresponding
outside Blue event orientations. This information is not recoverable from
legality masks alone and is tracked explicitly by two five-bit masks.
...
def run(k,half_turn,budget,legal_ports=False):
    ...
    @lru_cache(None)
    def white_wins(a,b,left_used=0,right_used=0,left_blocked=0,right_blocked=0):
        counts['states']+=1
        ...
        for v in ports:
            ...
            if legal_ports:
                ...
                if (right_blocked if side else left_blocked)&marker:continue
            ...
            if not white_wins(*child,next_left,next_right,left_blocked,right_blocked):
                records[(a,b,left_used,right_used,left_blocked,right_blocked)]=dict(event='port',vertex=v,next_left=next_left,next_right=next_right)
                return False
        ...
        for v in blue:
            ...
            block_l=left_blocked;block_r=right_blocked
            if v in ports:
                vr=v//width;vs=int(v%width==k);vp=4-vr if half_turn and vs else vr
                if vs:block_r|=1<<vp
                else:block_l|=1<<vp
            ...
            if legal_ports:
                for port in ports:
                    ...
                    paired=4-pr if half_turn and pside else pr;marker=1<<paired
                    # White at this cap port only removes exterior B(z).
                    # It is safe whenever reflected exterior A(tau(z))
                    # was already forbidden, even if B(z) was still legal.
                    opposite_blocks=block_l if pside else block_r
                    opposite_used=left_used if pside else right_used
                    if ((left_used|right_used|opposite_blocks)&marker or
                            opposite_used&((marker<<1)|(marker>>1))):
                        available_special|=1<<port
            options=[u for u in range(size) if bb&available_special&(1<<u)]
            if bb>>mate[v]&1:
                options=[mate[v]]+[u for u in options if u!=mate[v]]
            chosen=None
            for u in options:
                if white_wins(aa&~(1<<u),bb&~closed[u],left_used,right_used,block_l,block_r):
                    chosen=u;break
            if chosen is None:
                records[(a,b,left_used,right_used,left_blocked,right_blocked)]=dict(event='blue',vertex=v,white_options=options)
                return False
            responses.append([v,chosen])
        records[(a,b,left_used,right_used,left_blocked,right_blocked)]=dict(replies=responses)
        return True

    try:answer=white_wins(a,b)
    except RuntimeError:answer='Unknown'
    prefix=[];state=(a,b,0,0,0,0)
    while state in records and records[state].get('event')=='port':
        v=records[state]['vertex'];prefix.append([v//width,v%width])
        state=(state[0]&~(1<<v),state[1]&~(1<<mate[v]),records[state]['next_left'],records[state]['next_right'],state[4],state[5])
...
if __name__=='__main__':
    ...
    Path(__file__).with_name(f'round3_dx_port_controller_{args.cap_width}_{int(args.half_turn)}_{int(args.legal_ports)}.json').write_text(json.dumps(result,indent=2)+'\n')
