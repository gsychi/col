...
int main(int argc,char**argv){int h=std::stoi(argv[1]),w=std::stoi(argv[2]);U budget=std::stoull(argv[3]);S s(h,w,budget);std::cout<<"h,w,a,b,result,visits,memo\n";
if(argc>=6){U a=std::stoull(argv[4]),b=std::stoull(argv[5]);int out=2;try{out=s.win(a,b)?1:0;}catch(Limit&){}std::cout<<h<<","<<w<<","<<a<<","<<b<<","<<out<<","<<s.visits<<","<<s.memo.size()<<std::endl;return 0;}
for(U b=0;b<=s.full;b++){if(s.memo.size()>3000000)s.memo.clear();s.visits=0;int out=2;try{out=s.win(s.full,b)?1:0;}catch(Limit&){}std::cout<<h<<","<<w<<","<<s.full<<","<<b<<","<<out<<","<<s.visits<<","<<s.memo.size()<<std::endl;}
}