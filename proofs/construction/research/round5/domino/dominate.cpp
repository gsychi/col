...
int main(int argc, char** argv) {
    ...
    long nW = 0, okW = 0, nB = 0, okB = 0, nPF = 0, okPF = 0;
    int shown = 0;
    std::vector<int> P, Q;
    auto eval = [&]() {
        ...
        u64 occ = blue | white;
        i64 F = (i64)(__builtin_popcountll(majm & ~occ & fullmask()) - __builtin_popcountll(~majm & ~occ & fullmask())) * ONE;
        Val g = value(a, b); ++nPF; if (g.e == 0 && g.x == F) ++okPF;
        for (u64 z = b & majm; z; z &= z - 1) {
            int y = __builtin_ctzll(z); u64 yb = u64{1} << y;
            Val o = value(a & ~yb, b & ~NB[y]); ++nW;
            if (geq(o, F + ONE)) ++okW;
            else if (shown < 15) { ++shown; show(a, b); std::printf("  F=%lld White off-parity (%d,%d) -> %s\n", (long long)(F / ONE), y / W, y % W, fmt(o).c_str()); }
        }
        for (u64 t = a & ~majm; t; t &= t - 1) {
            int zc = __builtin_ctzll(t); u64 zb = u64{1} << zc;
            Val o = value(a & ~NB[zc], b & ~zb); ++nB;
            if (leq(o, F - ONE)) ++okB;
            else if (shown < 30) { ++shown; show(a, b); std::printf("  F=%lld Blue off-parity (%d,%d) -> %s\n", (long long)(F / ONE), zc / W, zc % W, fmt(o).c_str()); }
        }
    };
    ...
    recP(recP, 0);
    std::printf("%dx%d (|P|<=%d, %d<=|Q|<=%d): PF %ld/%ld; (W) G^{R,y} >= F+1: %ld/%ld; (B) G^{L,z} <= F-1: %ld/%ld\n",
                h, w, pmax, q0, qmax, okPF, nPF, okW, nW, okB, nB);
}
