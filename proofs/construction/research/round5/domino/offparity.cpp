...
int main(int argc, char** argv) {
    ...
    long cases = 0, adjAlways = 0, anyGood = 0, adjSomeGood = 0, majGood = 0;
    ...
    auto eval = [&]() {
        ...
        for (u64 z = b & majm; z; z &= z - 1) {
            ...
            ++cases;
            bool any = false, allAdj = true, someAdj = false, maj1 = false; int nadj = 0;
            ...
            if (maj1) ++majGood;
            if (!any && shown < 10) { ++shown; show(a1, b1); std::printf("  after White off-parity: no Blue reply reaches F\n"); }
        }
        ...
        for (u64 t = a & ~majm; t; t &= t - 1) {
            ...
            if (min1) ++minGoodB;
            if (!any && shown < 20) { ++shown; show(a1, b1); std::printf("  after Blue off-parity: no White reply reaches F\n"); }
        }
    };
