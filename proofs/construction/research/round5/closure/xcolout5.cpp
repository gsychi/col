// xcolout5: copy of ground_truth/colout5.cpp for the exact closure engine. Only
// change: transposition-table keys of multi-component nodes include the board
// width (bits 56..59), so one table can be shared by searches on boards of
// different widths. Valid only for boards of at most 56 cells.
//
...
struct Search {
    ...
    inline int quick(Mask A, Mask B, i64 q, bool s) {
        ...
        else { k1 = f.HA | (Mask(bd.w) << 56) | (Mask(15) << 60); k2 = f.HB | (Mask(15) << 60); }
        int bc;
    ...
    int win(Mask A, Mask B, i64 q, bool s, int depth, int et = -1) {
        ...
        else { k1 = HA | (Mask(bd.w) << 56) | (Mask(15) << 60); k2 = HB | (Mask(15) << 60); xf = Xf{0, 0, bd.h, bd.w, 0, 0}; }
        int bestCode;
