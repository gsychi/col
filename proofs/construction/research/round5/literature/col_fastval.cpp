...
int main(int argc, char** argv) {
    if (argc < 3) { std::fprintf(stderr, "see header for usage\n"); return 1; }
    std::string mode = argv[1];
    if (mode == "ddreserve") {
        // D = obwbo at both ends of a 5 x n strip, neutral elsewhere
        int maxn = std::atoi(argv[2]);
        for (int n = 1; n <= maxn; ++n) {
            ...
            Val g = value(a, b);
            int v = 2 * n + 0;  // (2,0) is White-only
            Val child = value(a & ~(u64{1} << v), b & ~NB[v]);
            std::printf("DD_%d = %s ; after White (2,0): %s ; reserve bound gives DD <= %s\n",
                        n, fmt(g).c_str(), fmt(child).c_str(), fmt(Val{child.x - ONE, child.e}).c_str());
            std::fflush(stdout);
        }
        return 0;
    }
    ...
    if (mode == "value") {
        ...
        show(a, b); std::printf(" = %s\n", fmt(value(a, b)).c_str());
    } else if (mode == "random") {
        int count = std::atoi(argv[4]); unsigned seed = (unsigned)std::atoi(argv[5]);
        std::srand(seed);
        for (int i = 0; i < count; ++i) {
            u64 a = 0, b = 0;
            for (int v = 0; v < N; ++v) {
                int k = std::rand() % 6;  // o o o b w .
                if (k <= 3) a |= u64{1} << v;
                if (k <= 2 || k == 4) b |= u64{1} << v;
            }
            std::printf("%llu %llu %s\n", (unsigned long long)a, (unsigned long long)b, fmt(value(a, b)).c_str());
        }
    } else if (mode == "openings") {
        Val e = value(full, full);
        std::printf("empty %dx%d = %s\n", h, w, fmt(e).c_str());
        for (int v = 0; v < N; ++v) {
            int r = v / W, cc = v % W;
            if (r > (H - 1) / 2 || cc > (W - 1) / 2) continue;
            Val o = value(full & ~NB[v], full & ~(u64{1} << v));
            std::printf("opening (%d,%d): %s\n", r, cc, fmt(o).c_str());
        }
        std::printf("memo %zu\n", memo.size());
    } else if (mode == "isolate") {
        // relative to centre (r0,c0): Blue (-1,-1), (1,-1), (0,2) if W >= 5, else transposed
        int r0 = H / 2, c0 = W / 2;
        std::vector<std::pair<int,int>> seq;
        if (W >= 5) seq = {{-1, -1}, {1, -1}, {0, 2}};
        else seq = {{-1, -1}, {-1, 1}, {2, 0}};
        u64 blue = 0, white = 0;
        for (auto [dr, dc] : seq) {
            int v = (r0 + dr) * W + (c0 + dc);
            blue |= u64{1} << v; white |= u64{1} << rho(v);
        }
        u64 a, b; perms_from_stones(blue, white, a, b);
        show(a, b); std::printf("  value after three mirrored pairs: %s\n", fmt(value(a, b)).c_str());
    } else if (mode == "mirror") {
        std::vector<int> reps;
        for (int v = 0; v < N; ++v) if (v < rho(v) && !((NB[c] >> v) & 1)) reps.push_back(v);
        long total = 0, neg = 0, star = 0, zero = 0, other = 0, notdom = 0;
        auto visit = [&](u64 blue, u64 white) {
            u64 a, b; perms_from_stones(blue, white, a, b);
            if (!((a >> c) & 1)) return;
            ++total;
            Val e = value(a, b);
            Val vc = value(a & ~NB[c], b & ~(u64{1} << c));
            if (vc.e == 0 && vc.x == 0) ++zero; else if (vc.e == 1 && vc.x == 0) ++star;
            else if (vc.x < 0) ++neg; else ++other;
            bool dom = false;
            for (u64 z = a & ~(u64{1} << c); z && !dom; z &= z - 1) {
                int v = __builtin_ctzll(z);
                if (leqv(vc, value(a & ~NB[v], b & ~(u64{1} << v)))) dom = true;
            }
            if (!dom) ++notdom;
            if ((vc.e == 0 && vc.x == 0) && zero <= 3) {
                std::printf("centre child = 0 (mirror fails): "); show(a, b); std::printf("  E'=%s\n", fmt(e).c_str());
            }
        };
        auto dfs = [&](auto&& self, std::size_t i, u64 blue, u64 white) -> void {
            ...
            for (int side = 0; side < 2; ++side) {
                ...
                if (NB[bv] & ~(u64{1} << bv) & blue) continue;
                self(self, i + 1, blue | (u64{1} << bv), white | (u64{1} << wv));
            }
        };
        dfs(dfs, 0, 0, 0);
        std::printf("mirror positions with live centre: %ld; centre child: negative %ld, star %ld, zero %ld, other %ld; "
                    "centre move not dominated by another Blue move: %ld\n", total, neg, star, zero, other, notdom);
        std::printf("memo %zu\n", memo.size());
    } else {
