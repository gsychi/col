...
> **Later theorem:** The all-width empty `3 × n` theorem was subsequently
> proved by the gadget-and-strong-induction argument in
> [`../empty_3xn_theorem.md`](../empty_3xn_theorem.md). Statements below saying
> that the `4k+3` empty-board family remains open describe the state at the
> time of these experiments and are superseded for current theorem status. The
> experiments and their bounded-search conclusions remain historical results.
